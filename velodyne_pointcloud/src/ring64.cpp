...
ros::Publisher object_distance_min_pub;
...
void callback(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  int number_of_col = 2452;
  int no_channels=64;
  Point_ring_phi array_P_r_phi[no_channels][number_of_col];
  ...
  for (int i = 0; i < no_channels; i++)
    ...
    //for (int j=0;j<100;j++)
    {
      ...
      //threshold=0.5+array_P_r_phi[i][j].range*(10-0.5)/100;
       // threshold=0.25;
      ...
      if (array_P_r_phi[i][j].label == 0 && array_P_r_phi[i][j].range != 0)
      {
        ...
        while (!myqueue.empty())
        {
          ...
          /***********DOWN NEIGHBOR*****************************/
          if (temp_pair.first < no_channels-1)
          {
           int temp = temp_pair.first + 1;
            while (temp <= temp_pair.first + 5 && temp <= no_channels-1)
            {
           // if (array_P_r_phi[temp_pair.first + 1][temp_pair.second].range != 0 && array_P_r_phi[temp_pair.first + 1][temp_pair.second].label == 0)
           if (array_P_r_phi[temp][temp_pair.second].range != 0 && array_P_r_phi[temp][temp_pair.second].label == 0)
            {
 ...
 /* for (int i = 0; i < vector_cluster.size(); i++)
  ...
  std::vector<double> vector_object_distances;
 ...
 //--------------------------------
   ...
   pt.x=vector_cluster[i].points[0].x;
   pt.y=vector_cluster[i].points[0].y;
   pt.z=vector_cluster[i].points[0].z;
   double position=sqrt(pow(pt.x,2)+pow(pt.y,2)+pow(pt.z,2));
   vector_object_distances.push_back(position);
 //double theta= atan2(pt.y - 0, pt.x - 0) * 180 / 3.141;
...
//   std::cout<<"angle "<<theta<<std::endl;
   ...
   //marker_text<<ss;
   std::string num_str1(std::to_string(position));
...
//----------------------------------------------------------------
 ...
 markerPub.publish(clusterMarkers);
  ...
  std::cout<<"bhaskar"<<std::endl;
  if (vector_object_distances.size()==0)
  {
  std::cout<<" no obstacle detected "<<std::endl;
  float min_distance_f=100;
  std_msgs::Float32 object_distance_msg;
  object_distance_msg.data=min_distance_f;
  object_distance_min_pub.publish(object_distance_msg);
  }
  else
  {
  double min_distance=*min_element(vector_object_distances.begin(),vector_object_distances.end());
  float min_distance_f=float (min_distance);
  std_msgs::Float32 object_distance_msg;
  object_distance_msg.data=min_distance_f;
  object_distance_min_pub.publish(object_distance_msg);
  }
...
{
  //Initiate ROS
  ros::init(argc, argv, "ground_removal_front");
  ros::NodeHandle nh;
  ...
  sub_ = nh.subscribe("/gremoved_front_view_topic", 1, callback);
  object_distance_min_pub = nh.advertise<std_msgs::Float32>("/lidar_min_distance", 10);
