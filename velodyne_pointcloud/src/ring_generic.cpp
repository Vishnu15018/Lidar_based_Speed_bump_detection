...
void callback(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  double horrizontal_resolution=0.2; // 32 channel lidar 0.2, 64 channel 0.08
  //int number_of_channel=32;
  int number_of_col = 65536 / number_of_channel;
  Point_ring_phi array_P_r_phi[number_of_channel][number_of_col];
  ...
  int counter[number_of_channel] = {0};
  ...
  for (size_t i = 0; i < cloud_XYZIR->points.size(); i++)
  {
    ...
    position = temp_phi / horrizontal_resolution;
    ...
    //for (int j=0;j<100;j++)
    {
      ...
      if (array_P_r_phi[i][j].label == 0 && array_P_r_phi[i][j].range != 0)
      {
        ...
        while (!myqueue.empty())
        {
          ...
          /***********DOWN NEIGHBOR*****************************/
          if (temp_pair.first < number_of_channel-1)
          {
           int temp = temp_pair.first + 1;
            while (temp <= temp_pair.first + 5 && temp <= number_of_channel-1)
            {
           // if (array_P_r_phi[temp_pair.first + 1][temp_pair.second].range != 0 && array_P_r_phi[temp_pair.first + 1][temp_pair.second].label == 0)
           if (array_P_r_phi[temp][temp_pair.second].range != 0 && array_P_r_phi[temp][temp_pair.second].label == 0)
            {
...
{
  //Initiate ROS
  std::cout<<"enter the number of channels for your LiDAR"<<std::endl;
  std::cin>>number_of_channel;
